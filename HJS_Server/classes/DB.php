...
class DB{
	...
	private function __construct($data = array()){
		try{
			if(!empty($data["host"]) AND !empty($data["database"]) AND !empty($data["username"]) AND !empty($data["password"])){
				$this->_pdo = new PDO("mysql:host=" . $data["host"] . "; dbname=" . $data["database"],$data["username"], $data["password"]);
			}else{
	...
	public static function conn($data = array()){
		if(!isset(self::$_instance)){
			self::$_instance = new DB($data);
		}
